...
/****************************************************************************************/
   ...
   register_widget( "zqh_posts_id_widget" );
/**
 * Featured Posts widget
 */
class zqh_posts_id_widget extends WP_Widget {

   function __construct() {
      $widget_ops = array( 'classname' => 'widget_highlighted_posts widget_featured_meta', 'description' => __( 'zqh_posts_id_widget', 'colormag') );
      $control_ops = array( 'width' => 800, 'height' =>445 );
      parent::__construct( false,$name= __( 'TG: zqh_posts_id', 'colormag' ),$widget_ops);
   }

   function form( $instance ) {
      $tg_defaults['number'] = "1,2,3,4";
      $instance = wp_parse_args( (array) $instance, $tg_defaults );
      $number = $instance['number'];
      ?>
      <p>
         <label for="<?php echo $this->get_field_id('number'); ?>"><?php _e( 'posts id:1,2,3:', 'colormag' ); ?></label>
         <input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="20" />
      </p>
      <?php
   }
   ...
   function widget( $args, $instance ) {
      ...
      global $post;
      $number = explode(",", $instance[ 'number' ]);
...
register_widget( "zqh_posts_id2_widget" );
/**
 * Featured Posts widget
 */
class zqh_posts_id2_widget extends WP_Widget {

   function __construct() {
      $widget_ops = array( 'classname' => 'widget_highlighted_posts widget_featured_meta', 'description' => __( '自定义文章显示id', 'colormag') );
      $control_ops = array( 'width' => 800, 'height' =>445 );
      parent::__construct( false,$name= __( 'TG: zqh_posts_id2', 'colormag' ),$widget_ops);
   }
...
register_widget( "zqh_highlighted_products_widget" );
/**
 * Highlighted Posts widget
 */
class zqh_highlighted_products_widget extends WP_Widget {

   function __construct() {
      $widget_ops = array( 'classname' => 'widget_highlighted_posts widget_featured_meta', 'description' => __( 'Display latest posts or posts of specific category. Suitable for the Area Beside Slider Sidebar.', 'colormag') );
      $control_ops = array( 'width' => 200, 'height' =>250 );
      parent::__construct( false,$name= __( 'TG: zqh_highlighted_products_widget', 'colormag' ),$widget_ops);
   }
